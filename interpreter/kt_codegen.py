# kt_codegen.py
# c++ code generator for the kt interpreter - generates and compiles c++ code from the program image generated by kt_compiler.py
...
class code_generator:
	...
	def emit_classes(self):
		def method_prototype(compound, slot):
			record = self.image.functions[slot.global_function_index]
			result = ""
			...
			if record.returns_value:
				...
				result += "void "
			result += slot.name + "(" + ", ".join( ("variable arg" + str(x) for x in range(record.arg_count) ) ) + ")"
			return result
		def method_body(compound, slot):
			record = self.image.functions[slot.global_function_index]
			result = ""
			if record.register_count:
				result += "\t\t\tvariable " + ", ".join( ("reg" + str(x) for x in range(record.register_count)) ) + ";\n"
		...
		decl_str = ""
		for compound in self.image.sorted_compounds:
	...
	def go(self):
		emit_string = "struct program\n{\n" + self.emit_classes() + "};\n"
		print emit_string
		return

		for o in (o for o in image.globals_list if o.type == 'object'):
