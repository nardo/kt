...
class compound_node (program_node):
	...
	def add_member_variable(self, element):
		...
		element.slot_index = self.slot_count
		self.slot_count += 1
		self.members[element.name] = element
	...
	def add_member_function(self, function_node):
		...
		if parent_func:
			...
			self.vtable_count += 1
		self.members[function_node.name] = slot(initial_node = self, slot_type = slot_types.function_slot_type, name = function_node.name, index = vtable_index, function_decl = function_node)
	...
	def analyze_compound(self, the_facet):
		if self.process_pass == 1:
			...
			if parent is not None:
				...
				if parent_node is None:
					...
					parent_node.analyze_compound(the_facet)
